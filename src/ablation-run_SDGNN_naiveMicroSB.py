...
def run_experiment(dataset_name='wiki',
    ...
    for run_id in range(num_splits):
        ...
        train_loss_dict[run_id] = {'supervised': [], 'microscale_sb': []}
        val_metrics_dict[run_id] = []

        # Adding social balance edges to training edges
        edges_labeled_by_sb = torch.hstack(signed_datasets[run_id]['unknowns'][:2])
        labels_by_sb = torch.zeros(edges_labeled_by_sb.shape[1]).to(device)
        for edge_idx in range(edges_labeled_by_sb.shape[1]):
            u, v = edges_labeled_by_sb[:, edge_idx]
            edge_clause = torch.logical_and(signed_datasets[run_id]['train']['masked_edges'][:, 0] == u,
                                            signed_datasets[run_id]['train']['masked_edges'][:, 1] == v)
            labels_by_sb[edge_idx] = signed_datasets[run_id]['train']['masked_label'][edge_clause].item()
        # how many of the labeled edges by social balance are correct?
        label_predicted_by_sb = torch.clone(signed_datasets[run_id]['unknowns'][2])
        label_predicted_by_sb[label_predicted_by_sb == -1.] = 0.
        ...
        for epoch in range(num_epochs):
            ...
            trust_all_loss = model.loss_sign.join_forward(zeta,
                                                          edges_labeled_by_sb,
                                                          label_predicted_by_sb,
                                                          reduction='none')
            microscale_sb_loss = trust_all_loss.mean()
            global_loss = loss + microscale_sb_loss
            # Perform backpropagation
            global_loss.backward()
            optimizer.step()
            train_loss_dict[run_id]['supervised'].append(loss.item())
            train_loss_dict[run_id]['microscale_sb'].append(microscale_sb_loss.item())
            if epoch % check_loss_freq == 0:
                ...
                with torch.no_grad():
                    ...
                    print(f'Microscale sb loss: {microscale_sb_loss.item()}')
                    print(
    ...
    for split_num in range(num_splits):
        ...
        fig = plot_losses(
            train_values=[train_loss_dict[split_num]['supervised'], train_loss_dict[split_num]['microscale_sb']],
            val_values=[val_metrics_dict[split_num]],
            train_labels=['supervised loss', 'microscale sb loss'],
