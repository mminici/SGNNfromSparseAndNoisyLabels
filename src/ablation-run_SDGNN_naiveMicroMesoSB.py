...
from data_processor import get_edge_index_with_sign, get_mesoscale_labels, create_noisy_labels_micro_and_meso
...
def run_experiment(dataset_name='wiki',
    ...
    # Start experiment
    # save parameters
    mlflow.log_param('model', 'Naive_MicroMeso_Split')
    ...
    for run_id in range(num_splits):
        ...
        # compute meso-scale labels
        masked_edges_label_mesoscale_sb = get_mesoscale_labels(signed_datasets[run_id], device)
        # move data to device
        ...
        f_dataset, f_labels = create_noisy_labels_micro_and_meso(signed_datasets[run_id], device,
        ...
        for epoch in range(num_epochs):
            ...
            noisy_loss = torch.mean(noisy_loss)  # re-weight the loss with the learned weights
