...
def run_experiment(dataset_name='wiki',
    ...
    # Start experiment
    # save parameters
    mlflow.log_param('model', 'PL_Rand')
        ...
        while pseudo_labeling_iter < num_max_iterations and enough_data_flag:
            ...
            with torch.no_grad():
                ...
                if num_neg_examples < num_sampled_items:
                    ...
                    pos_pl_idxs = np.random.choice(unsupervised_pos_idxs, num_sampled_items, replace=False)
                    pos_pl_idxs = torch.Tensor(pos_pl_idxs).to(device).long()
                    unsupervised_neg_idxs = np.array(range(len(pred_prob[~pred_label])))
                    neg_pl_idxs = np.random.choice(unsupervised_neg_idxs, num_sampled_items, replace=False)
                    ...
                    pos_idxs = torch.Tensor(
                        [i for i in range(unlabeled_edge_index[pos_edge_mask].shape[0]) if i not in pos_pl_idxs]).to(
                        device).long()
