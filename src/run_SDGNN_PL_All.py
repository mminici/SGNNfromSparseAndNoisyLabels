...
def run_experiment(dataset_name='wiki',
    ...
    # Create loggers
    train_logger_afterPL = TrainLogMetrics(num_splits, ['supervised'])
    ...
    for run_id in range(num_splits):
        ...
        with torch.no_grad():
            ...
            # Add pseudo-labeled unsupervised edges
            unsupervised_pos_idxs = np.array(range(len(pred_prob[pred_label])))
            pos_pl_idxs = torch.Tensor(unsupervised_pos_idxs).to(device).long()
            unsupervised_neg_idxs = np.array(range(len(pred_prob[~pred_label])))
            neg_pl_idxs = torch.Tensor(unsupervised_neg_idxs).to(device).long()
        ...
        for epoch in range(num_epochs):
            ...
            loss.backward()
            optimizer.step()
            train_logger_afterPL.train_update(run_id, 'supervised', loss.item())
            if epoch % check_loss_freq == 0:
                # Validation step
                model.eval()
                with torch.no_grad():
                    val_metrics = eval_model(model,
                                             ...
                                             signed_datasets[run_id]['train']['label'],
                                             signed_datasets[run_id]['val']['label'])
                    train_logger_afterPL.val_update(run_id, val_metrics[metric_to_optimize])
                    if val_metrics[metric_to_optimize] > BEST_VAL_METRIC:
                        BEST_VAL_METRIC = val_metrics[metric_to_optimize]
                        torch.save(model.state_dict(), best_model_path)
                        early_stopping_cnt = 0
                ...
                train_logger_afterPL.val_update(run_id, 0.0)
        # Test performance
        model.load_state_dict(torch.load(best_model_path))
        model.eval()
        with torch.no_grad():
